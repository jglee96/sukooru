import { defineComponent, mergeProps, unref, computed, toValue, ref, shallowRef, watch, inject, createElementBlock, getCurrentInstance, provide, cloneVNode, h, useSSRContext, onServerPrefetch, reactive, nextTick, toRef } from 'vue';
import { ssrRenderAttrs, ssrInterpolate, ssrRenderStyle, ssrRenderList } from 'vue/server-renderer';
import { debounce } from 'perfect-debounce';
import { b as useRoute, u as useNuxtApp, a as asyncDataDefaults, f as fetchDefaults, c as createError } from './server.mjs';
...
var FALLBACK_WARNING = "Sukooru could not access sessionStorage and is falling back to in-memory storage for this page session.";
var isQuotaExceededError2 = (error) => {
  return error instanceof DOMException && error.name === "QuotaExceededError";
};
var isWebStorage = (storage) => {
  return "getItem" in storage;
};
var readStorageKeys = (storage) => {
  ...
  return {
    get(key) {
      ...
      try {
        ...
        if (isQuotaExceededError2(error)) {
          throw error;
        }
...
var SUKOORU_KEY = /* @__PURE__ */ Symbol("sukooru");
var useSukooru = () => {
...
var useScrollRestore$1 = ({
  ...
  const resolveContainerId = () => {
    var _a;
    return (_a = toValue(containerId)) != null ? _a : "window";
  };
...
var useScrollRestore = (options = {}) => {
  const route = useRoute();
  const scrollKey = computed(() => {
    var _a;
    return (_a = toValue(options.scrollKey)) != null ? _a : route.fullPath;
  });
  return useScrollRestore$1({
    ...options,
    scrollKey
  });
};
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      }, _attrs))}><p>Status: ${ssrInterpolate(unref(status))}</p><ul style="${ssrRenderStyle({ "display": "grid", "gap": "12px", "padding": "0" })}"><!--[-->`);
      ssrRenderList(unref(products), (product) => {
        _push(`<li style="${ssrRenderStyle({ "border": "1px solid #d1d5db", "border-radius": "12px", "list-style": "none", "min-height": "96px", "padding": "16px" })}">${ssrInterpolate(product.name)}</li>`);
      });
      _push(`<!--]--></ul></main>`);
    };
  }
...
//# sourceMappingURL=products-CFUEl3Vz.mjs.map
